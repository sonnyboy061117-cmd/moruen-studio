...
async function testVersion3() {
  console.log('='.repeat(80));
  console.log('版本3单独测试：检查是否编造统计数据');
  console.log('='.repeat(80));
  console.log('\n原文（无统计数据）：');
  console.log(testArticle.substring(0, 200) + '...\n');
  console.log('='.repeat(80));

  const versionIndex = 2; // 版本3
  const usedOpenings = [];
  ...
  const strategyStart = lines.findIndex(line => line.includes('⚠️ 版本3'));
  if (strategyStart >= 0) {
    console.log(lines.slice(strategyStart, strategyStart + 10).join('\n'));
  }
  ...
  try {
    ...
    const hasNumberPattern = /(\d+成|\d+%|每\d+个|超过\d+|大概\d+|\d+个人中)/;
    const opening = result.substring(0, 100);

    if (hasNumberPattern.test(opening)) {
      console.log('⚠️  警告：开篇包含数字/统计表达');
      console.log('开篇内容：', opening);
      console.log('\n请确认这些数字是否来自原文，还是编造的。');
      console.log('原文统计数据：无');
    } else {
      console.log('✅ 通过：开篇未包含具体数字，使用现象切入');
      console.log('开篇内容：', opening);
    }
...
testVersion3().catch(console.error);
